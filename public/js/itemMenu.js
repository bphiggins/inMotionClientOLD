...
function removePreferredAccessoryByItem(hasUnitAccessoryTracking, localTransferNumber, prefix, unitCount, unit){
	...
	if (hasUnitAccessoryTracking == 0){
		if (unitCount == -1){
			...
			sql += "where localTransferNumber = ? ";
			sql += "and prefix= ? ";
			result = userDatabase.executeSql(sql, sqlExpression);
			if (result.length > 0){
		...
		for (i = 0; i < accessoriesList.length; i++){
			var accessoryPerUnitCount = accessoriesList[i].quantity / unitCount;
			var adjAccessoryQuantity = accessoryPerUnitCount * (unitCount - 1);
